...
my $role = undef;
my $email = undef;
my $schema = DataEntry::Auth::Schema->connect("dbi:SQLite:$Bin/../db/auth.db");
...
dataentry_auth_admin.pl - Sets the username and password for the generated authentication database

=head1 SYNOPSIS

dataentry_auth_admin.pl -user username -passwd password [-role role]
